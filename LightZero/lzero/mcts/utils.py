...
def generate_random_actions_discrete(num_actions: int, action_space_size: int, num_of_sampled_actions: int,
    ...
    actions = [
        np.random.randint(0, action_space_size,
                          num_of_sampled_actions).reshape(-1)
    ...
    if reshape and num_of_sampled_actions > 1:
        actions = [action.reshape(num_of_sampled_actions, 1)
...
def prepare_observation(observation_list, model_type='conv'):
    ...
    """
    assert model_type in ['conv', 'mlp', 'conv_context',
                          'mlp_context'], "model_type must be either 'conv' or 'mlp'"
    ...
    if model_type in ['conv', 'conv_context']:
        if observation_array.ndim == 3:
            ...
            observation_array = observation_array.reshape(
                batch_size, stack_num * channels, width, height)
        ...
        if observation_array.ndim == 3:
            ...
            raise ValueError(
                "For 'mlp' model_type, the observation must have 3 dimensions [B, S, O]")
...
def plot_simulation_graph(env_root, current_step, graph_directory=None):
    edge_topology_list, node_id_list, node_topology_list = obtain_tree_topology(
        env_root)
    ...
    graph_path = graph_directory + 'simulation_visualize_' + \
        str(current_step) + 'step.gv'
    dot.format = 'png'
