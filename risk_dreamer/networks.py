...
class RSSM(nn.Module):
    ...
    def observe_seq(self, embed, action, is_first, state=None):
    ...
    def kl_loss(self, post, prior, free_bits, dyn_scale, rep_scale):
        kl_div_f = torchd.kl.kl_divergence
        def dist(x): return self.get_dist(x)
        def separate_grads(x): return {k: v.detach() for k, v in x.items()}

        rep_loss = value = kl_div_f(
            dist(post) if self._discrete else dist(post)._dist,
            dist(separate_grads(prior)) if self._discrete else dist(
                separate_grads(prior))._dist,
        )
        dyn_loss = kl_div_f(
            dist(separate_grads(post)) if self._discrete else dist(
                separate_grads(post))._dist,
        ...
        rep_loss = torch.clip(rep_loss, min=free_bits)
        dyn_loss = torch.clip(dyn_loss, min=free_bits)
