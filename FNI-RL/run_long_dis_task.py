...
parser.add_argument(
    "--env_name", default="TrafficEnv5-v0", help="name of the environment to run"
)
parser.add_argument("--seed", default=0)
parser.add_argument("--train_step", default=2000)
parser.add_argument("--T_horizon", default=3005)
...
def train():
    # env.start(gui=False)
    wandb.init(project="dreamer3", name="fnirl")
    alg_cfg = FNIRL.Config()
    alg = FNIRL(alg_cfg, env, state_dim=args.state_dim,
    ...
    for n_epi in range(args.train_step):
        score, v, v_epi, done, infraction_check_epi, navigation_check, collision = (
            alg.interaction_long_dis_task(
                args.T_horizon, score, v, v_epi, args.speed_range, n_epi
            )
        )
        ...
        if (n_epi + 1) % args.print_interval == 0:
            print(
                "# of episode :{}, avg score_v : {:.1f}".format(
                    n_epi + 1, score / args.print_interval
                )
            )
            ...
            print(
                "######cn & infraction & sn rates:",
                cn / args.print_interval,
                i_n / args.print_interval,
                sn / args.print_interval,
            )
