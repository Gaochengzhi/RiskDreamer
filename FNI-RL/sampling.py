...
class SampleBuffer(Module):
    ...
    def __init__(
        self, state_dim, action_dim, capacity, discrete_actions=False, device=device
    ):
        super().__init__()
        ...
        self._bufs = {}
        self.register_buffer("_pointer", torch.tensor(0, dtype=torch.long).to(device))
    ...
    def from_state_dict(cls, state_dict, device=device):
        ...
        assert set(state_dict.keys()) == {
            *(f"_{name}" for name in cls.COMPONENT_NAMES),
            "_pointer",
        }
    ...
    def extend(self, **kwargs):
        ...
        ), "We do not support extending by more than buffer capacity"
        i = self._pointer % self.capacity
        end = i + batch_size
    ...
    def sample(self, batch_size, replace=True, device=device, include_indices=False):
        indices = (
            torch.randint(len(self), [batch_size], device=device)
            if replace
            else random_indices(len(self), size=batch_size, replace=False)
        )
