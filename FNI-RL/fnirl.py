...
class FNIRL(Configurable, Module):
    ...
    def __init__(self, config, env, state_dim, action_dim):
        ...
        self.model_ensemble = BatchedGaussianEnsemble(
            self.model_cfg, self.state_dim, self.action_dim
        )
    ...
    def interaction(
        ...
        for t in tqdm(range(max_episode_steps), desc=f"Episode {n_epi+1}"):
            if n_epi > warm_up:
                if t == 0:
                    self.update_world_model(self.model_steps)
                self.rollout_and_update()

            mu, _, pi = self.actor(torch.tensor(state).float().to(device))
            action = max_a * pi.item()

            r, next_s, done, info = self.real_env.step(action)
            for buffer in [episode, self.replay_buffer]:
                buffer.append(
                    states=torch.tensor(state).to(device),
                    actions=pi.detach(),
                    next_states=torch.tensor(next_s).to(device),
                    rewards=r / scale,
                    dones=done,
                    violations=done,
                )
        ...
        if (n_epi + 1) % 100 == 0 and 0:
            self.solver.save_model(int(score), self.real_env.label)
    ...
    def interaction_long_dis_task(
        ...
        for t in tqdm(range(max_episode_steps), desc=f"Episode {n_epi+1}"):
            ...
            (
                ...
                info,
            ) = self.real_env.step(pi)

            collision = info["collision"]
            infraction_check = info["collision"]
            navigation_check = info["navigation_check"]
            for buffer in [episode, self.replay_buffer]:
                buffer.append(
                    states=torch.tensor(state).to(device),
                    actions=pi.detach(),
                    ...
                    rewards=r / scale,
                    dones=done,
                    violations=(collision or infraction_check),
                )
    ...
    def rollout(self, policy, adv_policy, initial_states=None, k=0.9, c=0.1):
        if initial_states is None:
            initial_states = random_choice(
                self.replay_buffer.get("states"), size=self.rollout_batch_size
            )
        ...
        for t in range(self.horizon):
            with torch.no_grad():
                ...
                next_states, costs, uncertainty = self.model_ensemble.sample(
                    states, mix_actions
                )
        ...
        return (
            k * torch.mean(violations.float())
            + (1 - k) * torch.clamp(uncertainty, 0.0, 1.0)
        ).requires_grad_(True)
    ...
    def process_inputs(self, s, s_mean, s_std, s_epsilon):
        return (torch.tensor(s).to(device) - s_mean) / (s_std + s_epsilon)

    def save_model(self, model_name):
        name = "./models/" + self.real_env.label + "/env_model%d" % model_name
        torch.save(
            [
                self.model_ensemble.state_dict(),
                self.model_ensemble.state_normalizer.mean,
                self.model_ensemble.state_normalizer.std,
                self.model_ensemble.state_normalizer.epsilon,
            ],
            "{}.pkl".format(name),
        )
