...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        self._model_opt = util.Optimizer(
            "model",
            self.parameters(),
            config.model_lr,
            config.opt_eps,
        ...
        print(
            f"Optimizer model_opt has {
                sum(param.numel() for param in self.parameters())} variables."
        )
...
class ImagBehavior(nn.Module):
    def __init__(self, config, world_model):
        ...
        self._actor_opt = util.Optimizer(
            "actor",
        ...
        print(
            f"Optimizer actor_opt has {
                sum(param.numel() for param in self.actor.parameters())} variables."
        )
        self._value_opt = util.Optimizer(
            "value",
        ...
        print(
            f"Optimizer value_opt has {
                sum(param.numel() for param in self.value.parameters())} variables."
        )
    ...
    def _train(
        ...
        with util.RequiresGrad(self.actor):
            with torch.autocast('cuda', enabled=self._use_amp):
        ...
        with util.RequiresGrad(self.value):
            with torch.autocast('cuda', enabled=self._use_amp):
        ...
        metrics.update(util.tensorstats(value.mode(), "value"))
        metrics.update(util.tensorstats(target, "target"))
        metrics.update(util.tensorstats(reward, "imag_reward"))
        if self._config.actor["dist"] in ["onehot"]:
            ...
            metrics.update(util.tensorstats(imag_action, "imag_action"))
    ...
    def _imagine(self, start, policy, horizon):
        ...
        def step(prev, _):
            state, _, _ = prev
            feat = dynamics.get_feature(state)
            inp = feat.detach()
            action = policy(inp).sample()
            succ = dynamics.img_step(state, action)
            return succ, feat, action

        succ, feats, actions = util.static_scan(
    ...
    def _compute_target(self, imag_feat, imag_state, reward):
        if "cont" in self._world_model.heads:
            inp = self._world_model.dynamics.get_feature(imag_state)
            discount = self._config.discount * \
                self._world_model.heads["cont"](inp).mean
        else:
            discount = self._config.discount * torch.ones_like(reward)
        value = self.value(imag_feat).mode()
        target = util.lambda_return(
            reward[1:],
            value[:-1],
            discount[1:],
            bootstrap=value[-1],
    ...
    def _compute_actor_loss(
        ...
        if self._config.reward_EMA:
            ...
            metrics.update(util.tensorstats(normed_target, "normed_target"))
