...
ROOT_PATH = "/Users/kounarushi/mycode/TPAMI"
...
class Highway_env(gym.Env):
    ...
    def get_reward(self, veh_list):
        ...
        speed_reward = v_ego / self.maxSpeed - 0.01
    ...
    def step(self, action):
        ...
        try:
            ...
            smoothed_action = 0.1 * self.prev_action + 0.9 * action
            self.prev_action = smoothed_action
            acc, lane_change = smoothed_action[0].item(), smoothed_action[1].item()
            control_acc = self.max_acc * acc
            if "Auto" in traci.vehicle.getIDList():
                traci.vehicle.changeSublane(self.ego_id, lane_change)
                traci.vehicle.setAcceleration(self.ego_id, control_acc, duration=0.1)
            ...
            # reward -= 0.0001 * self._compute_soomth_loss(action, reward)
    ...
    def _load_simulation(self):
        ...
        traci.load(["-c", self.config_path, "--seed", str(np.random.randint(0, 10000))])
