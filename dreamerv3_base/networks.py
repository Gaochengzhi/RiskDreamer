...
class RSSM(nn.Module):
    def __init__(
        ...
        input_layers = []
        if self._discrete:
            input_dim = self._stoch * self._discrete + num_actions
        else:
            input_dim = self._stoch + num_actions
        input_layers.append(nn.Linear(input_dim, self._hidden, bias=False))
        if norm:
            input_layers.append(nn.LayerNorm(self._hidden, eps=1e-03))
        input_layers.append(act())
        self._img_in_layers = nn.Sequential(*input_layers)
        self._img_in_layers.apply(tools.weight_init)
        self._cell = GRUCell(self._hidden, self._deter, norm=norm)
        self._cell.apply(tools.weight_init)

        img_out_layers = []
        input_dim = self._deter
        img_out_layers.append(nn.Linear(input_dim, self._hidden, bias=False))
        if norm:
        ...
        obs_out_layers = []
        input_dim = self._deter + self._embed
        obs_out_layers.append(nn.Linear(input_dim, self._hidden, bias=False))
        if norm:
