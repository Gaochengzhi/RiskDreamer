...
def update_risk(cache):
    for env_id, transitions in cache.items():
        if not transitions:
            continue

        # 假设 transitions 是一个字典，其键是属性名，其值是包含这个属性值的列表
        risk_list = transitions['risk']
        is_first_list = transitions['is_first']

        # 找到风险值为1.0的最后一个索引
        index_to_modify = -1

        for i in range(len(risk_list) - 1, -1, -1):
            if risk_list[i] == np.array(1.0):
                index_to_modify = i
                break
        ...
        if index_to_modify >= 0:
            ...
            count = 0
            for j in range(index_to_modify, -1, -1):
                if not is_first_list[j]:
                    indices_to_modify.append(j)
                    count += 1
                    if count >= 20:
                        break
                else:
            ...
            # 开始对找到的索引进行风险值递增
            indices_to_modify.reverse()  # 反转顺序以从后向前递增
            increment_step = np.array(1.0 / max(1, len(indices_to_modify)))

            for k, index in enumerate(indices_to_modify):
                risk_list[index] = increment_step * k
...
def simulate(
    ...
    with tqdm.tqdm(total=steps, initial=step) as pbar:
        # steps : eval_every
        while (steps and step < steps) or (eval_episodes and episode < eval_episodes):
            # reset envs if necessary
            if done.any():
                indices = [
                    done_index for done_index, done_eps in enumerate(done) if done_eps
                ]
                infos = [envs[i].get_info() for i in indices]
                infos = [r() for r in infos]
                if infos[0].get("navigation") > 0.1:
                    wandb.log(infos[0])
                results = [envs[i].reset() for i in indices]
                results = [r() for r in results]
                for index, result in zip(indices, results):
                    t = {k: convert(v) for k, v in result.copy().items()}
            ...
            # action = {"action": action, "logprob": logprob}, agent_state: (latent, action)
            ...
            for a, result, env in zip(action, results, envs):
                ...
                if isinstance(a, dict):
                    a["action"] = info.get("action")
                    transition.update(a)
                else:
                    transition["action"] = info.get("action")
                transition["reward"] = r
                transition["risk"] = info.get("discount", np.array(float(d)))
            ...
            if done.any():
                ...
                for i in indices:
                    ...
                    if not is_eval:
                        erase_overflow_episodes(cache, limit)
    if is_eval:
...
def erase_overflow_episodes(cache, dataset_size):
    step_in_dataset = 0
...
def load_episodes(directory, limit=None, reverse=True):
    ...
    if reverse:
        for filename in reversed(sorted(directory.glob("*.npz"))):
            ...
            episodes[str(os.path.splitext(
                os.path.basename(filename))[0])] = episode
...
class OneHotDist(torchd.one_hot_categorical.OneHotCategorical):
    def __init__(self, logits=None, probs=None, unimix_ratio=0.0):
        if logits is not None and unimix_ratio > 0.0:
            ...
            probs = probs * (1.0 - unimix_ratio) + \
                unimix_ratio / probs.shape[-1]
...
class DiscDist:
    ...
    def log_prob(self, x):
        x = self.transfwd(x)
        # x(time, batch, 1)
        below = torch.sum((self.buckets <= x[..., None]).to(
            torch.int32), dim=-1) - 1
        ...
        dist_to_below = torch.where(
            equal, 1, torch.abs(self.buckets[below] - x))
        dist_to_above = torch.where(
            equal, 1, torch.abs(self.buckets[above] - x))
...
class UnnormalizedHuber(torchd.normal.Normal):
    ...
    def log_prob(self, event):
        return -(
            torch.sqrt((event - self.mean) ** 2 +
                       self._threshold**2) - self._threshold
        )
...
class SafeTruncatedNormal(torchd.normal.Normal):
    ...
    def sample(self, sample_shape):
        ...
        if self._clip:
            clipped = torch.clip(event, self._low +
                                 self._clip, self._high - self._clip)
...
def lambda_return(reward, value, pcont, bootstrap, lambda_, axis):
    ...
    returns = static_scan_for_lambda_return(
        lambda agg, cur0, cur1: cur0 + cur1 *
        lambda_ * agg, (inputs, pcont), bootstrap
    )
    if axis != 0:
...
class Optimizer:
    def __init__(
        ...
        self._opt = {
            ...
            "adam": lambda: torch.optim.AdamW(parameters, lr=lr, eps=eps),
            "nadam": lambda: NotImplemented(f"{opt} is not implemented"),
            "adamax": lambda: torch.optim.Adamax(parameters, lr=lr, eps=eps),
            "sgd": lambda: torch.optim.SGD(parameters, lr=lr),
            "momentum": lambda: torch.optim.SGD(parameters, lr=lr, momentum=0.9),
        }[opt]()
        self._scaler = torch.GradScaler(device="cuda", enabled=use_amp)
